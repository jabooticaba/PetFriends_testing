...
class StoreStuff:
  ...
  # something_to_start_with = "value"  #TODO Delete if usless
  auth_key = None
  pet_id = None
...
def test_add_new_pet_with_valid_data_and_photo(name = "Пушистик", animal_type = "Собака", age = '4', pet_photo = "../images/animal.jpg"):
  # _, auth_key = pf.get_api_key(valid_email, valid_password)
...
def test_update_information_about_pet_with_valid_data(name='Пушистик', animal_type="Свинка", age = '4'):
  status, result = pf.update_pet_info(store_stuff.auth_key, store_stuff.pet_id, name, animal_type, age)
  assert status == 200
  assert result['animal_type'] == 'Свинка'

# def test_delete_pet_valid_id():  # Удалить 10 питомцев в цикле pytest --repeat-scope=session --count=10, закомментить тесты 3, 4
